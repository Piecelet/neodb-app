...
@MainActor
public struct MastodonStatusRowView: View {
    ...
    @State public var viewModel: MastodonStatusRowViewModel
    ...
    public var body: some View {
        HStack(spacing: 0) {
                    ...
                    if !isCompact && context != .detail {
                        ...
                        .padding(
                            ...
                            theme.avatarPosition == .top
                                ? 0
                                : AvatarView.FrameConfig.status.width
                                    + .statusColumnsSpacing)
                    }
                    HStack(alignment: .top, spacing: .statusColumnsSpacing) {
                            ...
                            theme.avatarPosition == .leading
                        {
                            AvatarView(viewModel.finalStatus.account.avatar)
                                ...
                                .hoverEffect()
                                .onTapGesture {
                                    viewModel.navigateToAccountDetail(
                                        account: viewModel.finalStatus.account)
                                }
                        }
                        VStack(
                            alignment: .leading,
                            spacing: .statusComponentSpacing
                        ) {
                            if !isCompact {
                            ...
                            if !reasons.contains(.placeholder),
                                ...
                                isFocused
                                    || theme.statusActionsDisplay != .none,
                                !isInCaptureMode
                            {
                                StatusRowActionsView(
                                    isBlockConfirmationPresented:
                                        $isBlockConfirmationPresented,
                                    viewModel: viewModel
                                )
            ...
            .padding(
                .init(
                    top: isCompact ? 6 : 12, leading: 0,
                    bottom: isFocused ? 12 : 6, trailing: 0))
        }
        .onAppear {
            if !reasons.contains(.placeholder) {
        ...
        #else
            .listRowBackground(
                viewModel.makeBackgroundColor(isHomeTimeline: isHomeTimeline))
        #endif
        .listRowInsets(
            .init(
                top: 0,
        ...
        .accessibilityLabel(
            isFocused == false && accessibilityVoiceOverEnabled
                ? StatusRowAccessibilityLabel(viewModel: viewModel).finalLabel()
                : Text("")
        )
                ...
                Task {
                    do {
                        let operationAccount =
                            viewModel.status.reblog?.account
                            ?? viewModel.status.account
        ...
        .alert(
            ...
            .addTranslateView(
                isPresented: $viewModel.showAppleTranslation,
                text: viewModel.finalStatus.content.asRawText)
        #endif
    }
    ...
    private var accessibilityActions: some View {
        ...
        Button("settings.swipeactions.status.action.quote") {
            HapticManager.shared.fireHaptic(.notification(.success))
            viewModel.routerPath.presentedSheet = .quoteStatusEditor(
                status: viewModel.status)
        }
        .disabled(
            ...
            Button("accessibility.status.media-viewer-action.label") {
                ...
                let attachments = viewModel.finalStatus.mediaAttachments
                    ...
                    quickLook.prepareFor(
                        selectedMediaAttachment: attachments[0],
        ...
        Button("@\(viewModel.status.account.username)") {
            HapticManager.shared.fireHaptic(.notification(.success))
            viewModel.routerPath.navigate(
                to: .accountDetail(id: viewModel.status.account.id))
        }
        ...
        if viewModel.status.account != viewModel.finalStatus.account {
            Button("@\(viewModel.finalStatus.account.username)") {
                HapticManager.shared.fireHaptic(.notification(.success))
                viewModel.routerPath.navigate(
                    to: .accountDetail(id: viewModel.finalStatus.account.id))
            }
        }
        ...
        ForEach(viewModel.finalStatus.content.links) { link in
            ...
            case .url:
                if UIApplication.shared.canOpenURL(link.url) {
                    Button(
                        "accessibility.tabs.timeline.content-link-\(link.title)"
                    ) {
                        HapticManager.shared.fireHaptic(.notification(.success))
            ...
            case .hashtag:
                Button(
                    "accessibility.tabs.timeline.content-hashtag-\(link.title)"
                ) {
                    HapticManager.shared.fireHaptic(.notification(.success))
                    _ = viewModel.routerPath.handle(url: link.url)
                }
