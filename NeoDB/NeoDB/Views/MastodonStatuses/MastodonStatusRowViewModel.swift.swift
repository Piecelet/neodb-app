//
//  MastodonStatusRowViewModel.swift.swift
//  NeoDB
//
...
@MainActor
@Perceptible
class MastodonStatusRowViewModel {
    let status: Status
    ...
    private func recalcCollapse() {
        ...
        let showCollapseButton =
            ...
            && finalStatus.content.asRawText.unicodeScalars.count
                > collapseThresholdLength
        let newlineLimit =
            showCollapseButton && isCollapsed ? collapsedLines : nil
    ...
    var isThread: Bool {
        status.reblog?.inReplyToId != nil
            || status.reblog?.inReplyToAccountId != nil
    ...
    var homeBackgroundColor: some View {
        if status.visibility == .direct {
            ...
            if status.account.isPremiumAccount {
                makeDecorativeGradient(
                    startColor: .yellow, endColor: theme.primaryBackgroundColor)
            } else if userFollowedTag != nil {
                makeDecorativeGradient(
                    startColor: .teal, endColor: theme.primaryBackgroundColor)
            } else if status.reblog != nil {
                makeDecorativeGradient(
                    startColor: theme.tintColor,
                    endColor: theme.primaryBackgroundColor)
            } else {
    ...
    init(
        ...
        if status.mentions.first(where: {
            $0.id == CurrentAccount.shared.account?.id
        }) != nil {
            userMentionned = true
        } else {
    ...
    func navigateToDetail() {
        if isRemote, let url = URL(string: finalStatus.url ?? "") {
            ...
            routerPath.navigate(
                to: .statusDetailWithStatus(
                    status: status.reblogAsAsStatus ?? status))
        }
    }
    ...
    func loadAuthorRelationship() async {
        let relationships: [Relationship]? = try? await client.get(
            endpoint: Accounts.relationships(ids: [
                status.reblog?.account.id ?? status.account.id
            ]))
    ...
    func loadEmbeddedStatus() async {
        ...
        do {
            ...
            {
                embed = try await client.get(
                    endpoint: Statuses.status(id: String(id)))
            } else {
    ...
    func pin() async {
        ...
        do {
            let status: Status = try await client.post(
                endpoint: Statuses.pin(id: finalStatus.id))
    ...
    func unPin() async {
        ...
        do {
            let status: Status = try await client.post(
                endpoint: Statuses.unpin(id: finalStatus.id))
            updateFromStatus(status: status)
        } catch {
            isPinned = true
        }
    ...
    func delete() async {
        do {
            ...
            _ = try await client.delete(
                endpoint: Statuses.status(id: status.id))
        } catch {}
    }
    ...
    func fetchRemoteStatus() async -> Bool {
        guard isRemote, let remoteStatusURL = URL(string: finalStatus.url ?? "")
        else { return false }
        isLoadingRemoteContent = true
        let results: SearchResults? = try? await client.get(
            endpoint: Search.search(
